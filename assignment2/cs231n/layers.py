...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        std = np.sqrt(var + eps)
        out_i = (x-mu)/std
        out = gamma*out_i + beta
        cache = {"x":x, "mu": mu, "var": var, "std":std, "out_i": out_i, "gamma":gamma}
...
def batchnorm_backward(dout, cache):
    ...
    k = np.sum( dout *gamma * (x-mu), axis=0)
    k_star = -k/(2*np.power(std,3)*N)
    phi = dout * gamma/ std + k_star * 2 * (x- mu)
    dx = phi -np.sum(phi, axis=0)/N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x = np.pad(x,((0,0), (0,0), (pad_size, pad_size), (pad_size, pad_size)), mode='constant', constant_values=0)
    for f_ in range(F):
      for h_ in range(H_out):
        for w_ in range(W_out):
          out[:, f_ , h_ , w_ ] = np.sum(x[:, :, h_ *stride:h_ *stride+HH, w_ *stride: w_ *stride+WW]*w[f_ ], axis=(1,2,3)) + b[f_]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(output_height):
        for j in range(output_width):
            ...
            dx[:, :, i * stride: i * stride + filter_height, j * stride: j * stride + filter_width] += grad_x_slice
